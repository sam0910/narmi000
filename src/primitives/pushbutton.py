...
class Pushbutton:
    ...
    def __init__(self, pin, suppress=False, sense=None, lock=None, loop=None):
        ...
        self._run = loop.create_task(self._go(lock))  # Thread runs forever

    async def _go(self, lock=False):
        while True:
            # if self.lock:
    ...
    def _check(self, state):
        ...
        if state:  # Button pressed: launch pressed func
            ...
            self._state = state
            """Long press malfunction: sam's solution"""
            if self._ld:
                self._ld._ttask.cancel()
                self._ld.stop()
                self._ld.clear()
